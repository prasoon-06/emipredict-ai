...
from components.model_utils import load_regressor, applicant_to_frame, EMI_SCENARIOS

st.set_page_config(page_title="Predict Max EMI", page_icon="💰", layout="wide")
st.title("💰 Predict Maximum Safe Monthly EMI")

bundle = load_regressor()
if bundle is None:
    st.error("No trained regressor found. Run `python src/models/train_one.py reg ...` "
              "for each model, then `python src/models/finalize.py`.")
    st.stop()

pipeline, model_name = bundle["pipeline"], bundle["model_name"]
...
with st.form("emi_form"):
    ...
    st.subheader("Loan context (used for feature parity, doesn't affect the affordability estimate directly)")
...
if submitted:
    ...
    st.metric("Estimated maximum safe monthly EMI", f"₹{pred:,.0f}")
    proposed_emi = requested_amount / requested_tenure
    if proposed_emi > pred:
        st.warning(f"Requested EMI (₹{proposed_emi:,.0f}/mo) exceeds the estimated safe ceiling.")
    else:
        st.success(f"Requested EMI (₹{proposed_emi:,.0f}/mo) is within the estimated safe ceiling.")
